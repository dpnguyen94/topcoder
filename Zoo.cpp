...
long long Zoo::theCount(vector <int> a) {
	LL f[45];
	f[0] = 1;
	REP(i,1,45) f[i] = f[i - 1] * 2;
	
	int cnt[45];
	memset(cnt,0,sizeof(cnt));
	
	int n = sz(a), amax = *max_element(all(a));
	REP(i,0,n) cnt[a[i]] ++;
	FOR(i,0,amax) if (cnt[i] > 2) return 0;
	FOR(i,1,amax) if (cnt[i] > cnt[i - 1]) return 0;
	
	if (cnt[0] == 1) return 2;
	
	FOR(i,1,amax) if (cnt[i] == 1) return (f[i + 1]);
	return (f[amax + 1]);
}
